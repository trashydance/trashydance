...
export default function RoomLayout({ children }: { children: React.ReactNode }) {
	...
	const [userProfile, setUserProfile] = useState({ username: "You", avatarColor: "#c2f2d0" });
	...
	const sendMessage = (e?: React.FormEvent) => {
		...
		setChat([...chat, { user: userProfile.username || "You", text: message, color: userProfile.avatarColor || "#c2f2d0" }]);
		setMessage("");
	};
			...
			<div className="w-full md:w-1/3 min-w-[320px] h-[40vh] md:h-full flex flex-col bg-[#ffd1dc] border-4 border-black rounded-[40px] shadow-[12px_12px_0_0_rgba(0,0,0,1)] overflow-hidden shrink-0">
				...
				<div className="p-4 sm:p-6 border-b-4 border-black bg-white flex flex-col gap-4 shrink-0 z-10">
					<div className="flex items-center justify-between">
						<h2 className="font-black text-2xl sm:text-3xl uppercase tracking-widest text-black">Chat</h2>
						<div className="flex items-center gap-2">
							<span className="font-bold text-sm uppercase text-black/50 hidden lg:inline-block">Live</span>
					...
					<div className="flex items-center justify-center gap-3">
						{[...Array(4)].map((_, i) => {
							...
							return (
								<div key={i}
									className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full border-4 border-black flex items-center justify-center overflow-hidden ${i < playersCount ? 'shadow-[4px_4px_0_0_rgba(0,0,0,1)]' : 'opacity-50'}`}
									style={{ backgroundColor: isUser ? userProfile.avatarColor : (i < playersCount ? roomColor : '#e5e7eb') }}
								>
									{isUser ? (
										...
										<User className="w-5 h-5 sm:w-6 sm:h-6 text-black" strokeWidth={3} />
									)}
								</div>
							);
						})}
				...
				<div className="flex-1 p-6 overflow-y-auto flex flex-col gap-4 bg-[#ffd1dc] custom-scroll">
					{chat.map((msg, i) => {
						const isMe = msg.user === userProfile.username || msg.user === "You";
						return (
							<div key={i} className="flex flex-col">
								<span className="font-black uppercase text-sm mb-1 text-black/80">{msg.user}</span>
								<div
									className="border-4 border-black p-4 rounded-2xl shadow-[4px_4px_0_0_rgba(0,0,0,1)] text-black font-bold text-lg"
									style={{ backgroundColor: isMe ? userProfile.avatarColor : (msg.color || "white") }}
								>
									{msg.text}
								</div>
							</div>
