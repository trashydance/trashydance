				...
				<div className="relative flex justify-center items-center w-full max-w-[100vw] h-[350px] sm:h-[400px]">
					{GAMES.map((game, idx) => {
						...
						if (offset === 0) {
							...
							offset === -1 ||
							(currentIndex === 0 && idx === GAMES.length - 1)
						) {
							zIndex = 20;
							translateX = "-55%";
							scale = 0.8;
							...
							offset === 1 ||
							(currentIndex === GAMES.length - 1 && idx === 0)
						) {
							zIndex = 20;
							translateX = "55%";
							scale = 0.8;
						...
						return (
							<button
								...
								className={`absolute w-[280px] h-[320px] sm:w-[350px] sm:h-[350px] ${game.color} border-8 border-black shadow-[16px_16px_0_0_rgba(0,0,0,1)] flex flex-col items-center justify-center p-6 group transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] cursor-pointer text-left`}
								style={{
							...
							>
								...
								{offset === 0 && (
									<div className="mt-8 bg-white border-4 border-black px-8 py-3 text-xl sm:text-2xl font-black uppercase text-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] hover:-translate-y-2 hover:shadow-[8px_8px_0_0_rgba(0,0,0,1)] active:translate-y-1 active:translate-x-1 active:shadow-none transition-all">
										START
									</div>
								)}
							</button>
