...
export default function NewRoomPage() {
	...
	const handleCreate = (e: React.FormEvent) => {
		...
		const newRoom = {
			...
			players: 1,
			participants: [
				{ id: "admin", name: "You (Admin)", wins: 0 }
			]
		};
		...
		<div className="relative min-h-screen bg-white flex flex-col items-center justify-center p-4 sm:p-8 font-sans overflow-hidden transition-colors duration-500">
			...
			<div className="absolute top-4 right-4 sm:top-8 sm:right-8 z-10">
				<Link href="/rooms">
					...
					>
						<X className="w-6 h-6 text-black group-hover:scale-110 transition-transform" strokeWidth={3} />
					</button>
				</Link>
			</div>
			...
			>
				...
				<form onSubmit={handleCreate} className="flex flex-col gap-8">
					...
					<div className="flex flex-col gap-3">
						<label className="font-bold text-xl uppercase tracking-wide">Cloud Name</label>
						<input
							type="text"
							required
							maxLength={9}
					...
					{/* Cloud Color */}
					<div className="flex flex-col gap-3">
						<label className="font-bold text-xl uppercase tracking-wide">Cloud Color</label>
					...
					{/* Cloud Code */}
					<div className="flex flex-col gap-3">
						<label className="font-bold text-xl uppercase tracking-wide">Secret Code</label>
						<input
							type="text"
							required
							value={cloudCode}
					...
					>
						<span>Create</span>
						<ArrowRight className="w-8 h-8 text-black group-hover:translate-x-2 transition-transform" strokeWidth={4} />
					</button>
				</form>
