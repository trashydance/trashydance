...
import { Key, Plus, Settings, User, Users, X } from "lucide-react";
...
export default function RoomsPage() {
	...
	return (
      ...
      */}
			<div
				className={`fixed bottom-6 right-6 sm:bottom-12 sm:right-12 z-50 w-20 h-20 bg-white rounded-full pointer-events-none transition-transform duration-700 ease-in-out origin-center ${
					isCreating ? "scale-[150] opacity-100" : "scale-0 opacity-0"
				}`}
			/>
      ...
      */}
			...
			<div className="absolute top-4 right-4 sm:top-8 sm:right-8 z-10">
				<Link
					href="/settings"
					className="flex items-center justify-center w-12 h-12 bg-white border-2 border-black rounded-full shadow-[4px_4px_0_0_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0_0_rgba(0,0,0,1)] transition-all cursor-pointer group"
				>
					<Settings
						className="w-6 h-6 text-black group-hover:rotate-90 transition-transform duration-300"
						strokeWidth={2.5}
					/>
				</Link>
			</div>
      ...
      */}
			<div className="flex flex-col items-center justify-center min-h-[80vh] pt-20">
						...
						<div key={room.id} className="relative">
							...
							<Dialog>
								<DialogTrigger asChild>
									...
									>
										<h3 className="font-black text-2xl uppercase tracking-widest text-black mb-2">
											{room.name}
										</h3>
              ...
              */}
								<DialogContent
									...
									className="sm:max-w-2xl bg-[#fdfbf7] p-0 overflow-hidden outline-none"
									style={{
										borderRadius: "1.5rem",
								...
								>
									...
									<div className="flex flex-col md:flex-row h-full text-black">
										...
										<div className="flex-1 p-8 flex flex-col justify-between border-b-4 md:border-b-0 md:border-r-4 border-black">
											<DialogHeader>
												<DialogTitle className="text-4xl font-black uppercase tracking-wider text-black">
													{room.name}
												</DialogTitle>
											</DialogHeader>
                        ...
                        */}
													{[...room.participants]
														...
														.map((participant) => (
															<li
																key={participant.id}
																className="flex items-center gap-3 bg-white p-2 rounded-lg border-2 border-black"
															>
																...
																<div className="flex flex-col overflow-hidden">
																	...
																	<span className="text-xs text-black font-semibold text-gray-600">
			...
			<div className="absolute bottom-6 right-6 sm:bottom-12 sm:right-12 z-20 flex flex-col items-end gap-4">
				...
				<Dialog>
					...
					<DialogContent
						aria-describedby={undefined}
						className="sm:max-w-md bg-[#fdfbf7] p-8 outline-none"
						style={{
							borderRadius: "1.5rem",
							border: "4px solid black",
							boxShadow: "12px 12px 0px 0px rgba(0,0,0,1)",
					...
					>
						<DialogDescription className="sr-only">
							Enter specific room code to join.
						</DialogDescription>
						<DialogHeader>
						...
						<DialogFooter className="flex gap-4">
							<DialogTrigger asChild>
								<Button
									variant="outline"
									className="flex-1 border-2 border-black text-black font-bold uppercase py-6 rounded-xl hover:bg-gray-100 text-lg"
								>
									Cancel
								</Button>
							</DialogTrigger>
			...
			>
				<DialogContent
					aria-describedby={undefined}
					className="sm:max-w-sm bg-[#fdfbf7] p-8 outline-none"
					style={{
						borderRadius: "1.5rem",
						border: "4px solid black",
						boxShadow: "12px 12px 0px 0px rgba(0,0,0,1)",
				...
				>
					...
					<div className="py-4">
						<p className="text-black font-bold text-lg uppercase text-center">
