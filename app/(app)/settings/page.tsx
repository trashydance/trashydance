...
export default function SettingsPage() {
	...
	useEffect(() => {
		if (isProfileLoaded) {
			localStorage.setItem(
				"trashydance_user_profile",
				JSON.stringify({ username, avatarColor }),
			);
	...
	const fontClass =
		fontFamily === "leggibile"
			? "font-sans"
			: fontFamily === "creativo"
				? "font-serif"
				: "";
	const customFont =
		fontFamily === "simpatico"
		...
		>
			...
			<div className="fixed bottom-6 right-6 sm:bottom-10 sm:right-[10%] xl:right-[15%] z-50">
				...
				>
					...
					/>
					<span
						className={`font-black uppercase tracking-wider ${textSizeClass}`}
					>
						{t("Back")}
					</span>
				</button>
			...
			<button
				type="button"
				className="relative w-full max-w-sm h-20 bg-white border-4 border-black rounded-full shadow-[8px_8px_0_0_rgba(0,0,0,1)] flex cursor-pointer p-2 mb-12 shrink-0 select-none"
				onClick={() =>
					setActiveTab((prev) => (prev === "profile" ? "settings" : "profile"))
				}
			>
				<div
					className={`absolute top-2 bottom-2 w-[calc(50%-0.5rem)] bg-pink-500 border-4 border-black rounded-full transition-transform duration-300 shadow-[inset_-4px_-4px_0_rgba(0,0,0,0.2)] ${
						activeTab === "settings" ? "translate-x-full" : "translate-x-0"
					}`}
				/>
				<div
								...
								<div className="bg-[#e6e6fa] border-4 border-black rounded-[2rem] p-6 sm:p-10 shadow-[8px_8px_0_0_rgba(0,0,0,1)] flex flex-col gap-8 animate-in fade-in zoom-in-95 duration-300">
									...
									<div className="flex items-center gap-6 pb-4">
										<span className="font-bold uppercase text-2xl w-40">
											{isNightMode ? t("Night Mode") : t("Light Mode")}
										</span>
									...
									<div className="flex items-center justify-between gap-6 pb-4 bg-white p-6 rounded-2xl border-4 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
										<div className="flex flex-col">
											...
											<span className="font-bold opacity-50">
												{t("Replace all text with asterisks (*)")}
											</span>
										</div>
										<button
											type="button"
