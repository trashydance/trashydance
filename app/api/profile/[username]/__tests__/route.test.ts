...
describe("GET /api/profile/[username]", () => {
	it("returns 401 when not authenticated", async () => {
		...
		const res = await GET(req as any, makeParams("alice"));
	...
	it("returns 404 when user is not found", async () => {
		...
		const res = await GET(req as any, makeParams("nonexistent"));
	...
	it("returns 200 with profile and friend status", async () => {
		...
		const res = await GET(req as any, makeParams("bob"));
	...
	it('returns 200 with own profile when username is "me"', async () => {
		...
		const res = await GET(req as any, makeParams("me"));
		expect(res.status).toBe(200);
